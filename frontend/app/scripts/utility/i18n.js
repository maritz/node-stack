define(["libs/storage", "libs/sprintf-0.7-beta1", "underscore", "libs/jquery.jsperanto"], function (store, sprintf, _) {
  var i18n_options = {
    interpolationPrefix: '{',
    interpolationSuffix: '}',
    reusePrefix: "_(",
  ...
  , loadJSperanto = function (dict) {
    ...
    $.jsperanto.init(function(){ }, i18n_options);
  };
  ...
  jQuery.t = function(name, submodule, module) {
    ...
    if (t !== search_string) {
      if (values.length > 0) {
        return sprintf.vsprintf(t, values);
      } else {
        return t;
      }
    } else {
  ...
  try {
    ...
    if (localDict.hash !== window.i18n_hashes[lang]) // compare the local dictionary version hash with the one the server provided in the layout.jade
      throw '';
